...
class KeywordServicePage extends StatefulWidget {
  KeywordServicePage({Key key, this.title}) : super(key: key);
  ...
  @override
  _KeywordServicePageState createState() => new _KeywordServicePageState(title);
}

class _KeywordServicePageState extends State<KeywordServicePage> {
  String title;

  _KeywordServicePageState(this.title); //constructor
  ...
  Future<ServiceObject> makeRequest() async {
    ...
    var response = await http.get(Uri.encodeFull(url + "?keywords=%" + title),
        headers: {
          "Authorization": token,
